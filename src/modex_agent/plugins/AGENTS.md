<!-- Parent: ../AGENTS.md -->
<!-- Updated: 2026-09-10 | shell groups and assembly ownership -->
...
| ``Capability`` ABC + protocol types | ``capability.py`` | The cross-slot capability-bundle unit (ADR-0047, five-phase protocol: ``applies``/``contribute``/``bind`` compile-time, ``supply`` pool-assembly-time, ``assemble`` agent-assembly-time). Frozen view/contribution types: ``AgentDeclarationView``/``AgentDeclaredFields`` (C0 predicate input — declared state only), ``TreePositionView``, ``CapabilityContribution`` (tools/atomic ``tool_groups``/hooks/sections/``derived_tools``), ``PromptSectionSpec`` + ``SectionPlacement`` (fixed `HEAD`/`TAIL` anchors), ``CapabilityBinding`` (``active_sections`` + ``hooks`` vouching + payload), ``CapabilitySupply`` (pool-supply ABC with async ``start``/``stop`` lifecycle defaults), ``CapabilityWiring`` (prompt providers + per-agent artifacts), ``PoolSupplyView`` (pool-level (agent, config) entries + distilled resource fields), ``FinalRosterView`` (C2 input), ``DerivedToolSpec``/``DerivedToolOrigin``, ``CapabilityError``. Import-light: no runtime framework imports |
...
| ``AssemblySpec`` / ``MemoryOverrides`` | ``assembly/spec.py`` | Assembly input spec (per-agent and pool-level component-name references), compile-time atomic ``tool_groups`` candidate manifests, and per-agent memory config override. |
...
| ``AssembledAgent`` / ``AssemblyBuilder`` | ``assembly/builder.py`` | Output container + mutable accumulator with cleanup-on-failure; ``agent_resource_owner`` carries the Stage 3 prerequisite owner into native Stage 4. |
| ``AssemblyResourceOwner`` | ``assembly/resources.py`` | The single provisional owner for per-agent assembly resources. Adopt in dependency order, transfer once to ``AgentInstance``, clean in reverse order, and retain a failed dependent plus prerequisites for retry. ``assembly/interceptors.py`` adopts a local sandbox guard before shell construction. |
...
| ``defaults/capabilities/`` | ``defaults/capabilities/`` | Bundled implementations registered by ``register_default_capabilities``: ``aci``, ``ast_grep``, ``experience``, ``shell``, ``skills``, ``subagents``, ``todo``, and ``tracing``. `shell` is reusable explicit opt-in and owns the atomic `bash` group (see `defaults/capabilities/shell/AGENTS.md`); the bot overrides only its auto-application policy. `skills` auto-applies to every native agent, honors `capabilities: {skills: false}`, and is structurally excluded from external agents; its `SkillsSupply` owns one `SkillCatalog` per effective agent (see `defaults/capabilities/skills/AGENTS.md`). `subagents` auto-applies from tree position; the remaining bundles are declaration-driven. |
| ``NativeAssemblyInputs`` / ``NativeAssemblyResult`` / ``assemble_native_agent`` | ``assembly/native_core.py`` | Unified native-agent core — resolves the 5 per-agent slots (TOOL/LLM_PROVIDER/SYSTEM_PROMPT_PROVIDER/MEMORY_SYSTEM/HOOK), validates group products against compiled manifests, merges memory, dispatches hooks, constructs the descriptor, and calls ``agent_factory.create_agent``. ``resource_owner`` is the optional shared owner from earlier stages; absent means this entry creates one. Successful assembly transfers it to ``AgentInstance``; failures roll back through the same owner. Called by Stage 4, standalone assembly, and ``AgentTemplate.materialize``. |
...
- Per-agent resources use one ``AssemblyResourceOwner`` across interceptor and native-tool assembly. Preserve prerequisite order (guard before shell), transfer rather than copy, and never add a second cleanup callback for a resource already adopted there.
- A TOOL factory named by a group anchor returns one complete ``ToolGroup`` per assembly. Companion tools are neither registry entries nor separate factories; native assembly validates exact membership before transactional manager registration.
