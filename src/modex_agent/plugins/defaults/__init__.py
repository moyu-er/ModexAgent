...
8 ``register_default_*`` functions from the ``defaults`` subpackage. It is
...
The 8 ``register_default_*`` functions populate 7 of the 11
...
class DefaultPluginConfig(BaseModel):
    ...
    The 8 ``register_default_*`` functions take no construction-time
...
class DefaultPlugin(Plugin):
    """FW-bundled plugin aggregating all 8 ``register_default_*`` functions.
    ...
    via the 11 ``ctx.register_*`` methods; the ``PluginRegistrationContext``
    ...
    def register(self, ctx: PluginRegistrationContext) -> None:
        """Register all 8 default factory groups into *ctx*.
