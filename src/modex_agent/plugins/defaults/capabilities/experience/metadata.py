...
Regular runtime classes (plan §6.1): the store holds live file handles
via a path resolver. The record itself is a frozen Pydantic value.
"""
...
from modex_agent.plugins.defaults.capabilities.experience.paths import META_FILENAME
from modex_agent.utils.file_io import safe_atomic_replace

logger = logging.getLogger(__name__)


class ExperienceMetaRecord(BaseModel):
    ...
    def with_last_used(self, now: str) -> ExperienceMetaRecord:
        """Copy with ``last_used_at`` set (and ``created_at`` backfilled)."""
...
class ExperienceMetaStore(ABC):
    ...
    def touch(self, name: str) -> ExperienceMetaRecord:
        ...
        record = self.get(name) or ExperienceMetaRecord()
        record = record.with_last_used(_now_iso())
        self.set(name, record)
        return record
...
class PerFileExperienceMetaStore(ExperienceMetaStore):
    ...
    def set(self, name: str, record: ExperienceMetaRecord) -> None:
        self._write_file(self._meta_path(name), record)
    ...
    def _meta_path(self, name: str) -> Path:
        return self._root / name / META_FILENAME
    ...
    def _read_file(path: Path) -> ExperienceMetaRecord | None:
        try:
            ...
            if not isinstance(raw, dict):
                return None
            return ExperienceMetaRecord.model_validate(
                {
