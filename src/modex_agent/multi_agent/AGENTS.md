...
| File | Description |
|------|-------------|
| `pool.py` | `AgentPool` — resident-agent registry, the poll-driven inbox surface (`submit_input`, `consume_inbox`, `sessions_with_pending`, `dispatch_envelope`, `recover_parent_session`), session/task eviction, and the awaitable request-scope surface: `begin_request()` (two-phase admission token) and `run_input()` (activates the reservation, stamps the prompt with its scope, delivers through `submit_input`, waits on the tree's single quiesce loop for the frozen outcome). `dispatch_envelope` consumes the pipeline's typed `TurnOutcome` and notes the root/child fact into the request-scope owner from INSIDE the dispatch coroutine; the `tree` setter attaches the pool-owned approval terminator (`terminate_pending_approval`) used by cancel AND restart settlement. `input_message_from_dispatch_envelope` reconstructs the full `InputMessage` (content + `approval_decision` + `attachments_resolved`) from a broker envelope. |
| `inbox_poller.py` | `InboxPoller` — the sole between-turn driver (one per pool). Event-driven via a pool-level `asyncio.Event` signalled from `LocalAgentMessageBus.send` (the single convergence point of all inbox writers), with an `interval`-cadence tick as a defensive fallback for writers that bypass the bus. Owns `inflight: dict[sid, Task]` single-flight + `reconcile_inflight`; delegates per-envelope turn execution to `pool.dispatch_envelope`. Request-scoped trees: `dispatch_consume_types()` narrows an AWAITING_APPROVAL batch to decision prompts (causal carriers stay queued — no hold/release churn, no starvation: the filter precedes the batch limit) and `admit_scoped_dispatch()` returns `DispatchDecision` (DISPATCH / HOLD / ARCHIVE); a no-progress round suppresses its own wakeup so held work never spins the loop (interval tick or the next real signal retries it). |
...
| `session_tree/` (package) | `SessionTreeManager` — the tree owner: node/track/tree stores over the bus, `wait_quiesce` (the single completion loop), pause/binding, and the request-scope lifecycle (`request_scope.py`): `RequestScopeState`/`RequestOutcome`/`RequestScopeRecord` (persisted attribution fact in the root session's registry metadata), `RequestReservation`/`RequestResult`/`RequestTurnFact`/`DispatchDecision`, the error family (`RequestBusyError`/`ScopeRequiredError`/`ScopeMismatchError`/`ReservationLostError`), and `REQUEST_SCOPE_ID_KEY` — the source-scope stamp causal traffic carries (`SendStrategy.send` and `SubagentAutoSendHook` read `sender_scope_id()`). Scope surface: `register_request_scoped`, `begin_request`/`release_reservation`/`activate_reservation` (two-phase admission), `note_scope_turn` (facts never revive a CANCELLING scope), `wait_request` (terminal outcome persists BEFORE any waiter wakes), `cancel_request`, `settle_interrupted_scopes` (restart settlement — also terminates leftover approval batches, FAILED+raise on termination failure), `admit_scoped_input` (deliver gate) / `admit_scoped_dispatch` / `dispatch_consume_types` (turn admission). |
| `../core/agent.py` | Canonical `AgentCommKind` — `NORMAL` / `SUBAGENT` topology kind. |
