...
| `safety.py` | `SafetyConfig` — LLM safety timeouts (`LLMSafetyConfig`: `request_timeout`/`stream_idle_timeout` default `None` — no provider-level timeout, watchdog is sole termination mechanism) turn safety timeouts (`TurnSafetyConfig`: `hook_timeout`=10s, `tool_timeout`=540s per-invocation tool deadline), and unified watchdog tuning (`DeadlineSafetyConfig`: `chunk_renew_seconds`/`max_ahead_seconds`/`watchdog_poll_seconds`) |
