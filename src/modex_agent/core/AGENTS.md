...
| `capabilities.py` | `Modality` (StrEnum: TEXT/IMAGE/VIDEO/AUDIO), `ModelCapabilities` (frozen dataclass exposing modalities + `supports()`). Moved from `ioc/configs/llm.py` so `core/tool_manager` and `runtime/services` can reference them without upward ioc dependency; `ioc/configs/llm.py` re-exports for config-layer consumers |
...
| `tool_manager.py` | `Tool` class (dual-mode: `__init__` args OR `@property` name/description/parameters), `ToolManager` ABC, `InMemoryToolManager`, `ToolResult` (with optional `content_blocks` for multimodal output), `ToolExecutionContext` (frozen BaseModel delivered via contextvar — carries `ModelCapabilities` to tools), `get_tool_execution_context()`, `FunctionalTool`, `ToolConfig`, `ToolManagerConfig`, `ToolExecutionMode` |
