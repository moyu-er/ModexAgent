...
LRU eviction and memory-pressure buffer clearing on top of
``BaseTerminalManager``. ADR-0010 Decision 8 folds those capabilities inward;
they are now flag-guarded private methods on ``BaseTerminalManager`` via the
``max_terminals`` and ``enable_memory_pressure`` constructor parameters.
