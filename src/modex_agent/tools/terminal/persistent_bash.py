...
class PersistentBashTool(Tool):
    ...
    def description(self) -> str:
        ...
        return (
            ...
            "* STRICT resource limits: never launch more concurrent workers than the\n"
            "  machine's CPU cores and never let combined memory allocations approach\n"
            "  physical RAM — exceeding either gets the whole process OOM-killed and ALL\n"
            "  work is lost. Bound batch parallelism with\n"
            "  `for f in *; do cmd $f & (( ++n % $(nproc) == 0 )) && wait; done; wait`.\n"
            "* When an interactive program takes over the terminal (a remote login, a\n"
            "  REPL, a pager), the call returns promptly with a trailing [hint: ...]\n"
            "  line explaining who owns the terminal: for a remote shell or REPL keep\n"
