...
| `aci/` | Agent-Computer Interface enhancements — `edit_tool.py` (`AciEditTool`, EditFileTool + post-edit lint). Enabled via the `aci` capability (`capabilities: {aci: {}}` in the scope declaration — ADR-0047; contributes `aci_edit` and the `edit ← aci_edit` replacement record). Consumes `tools/lint/` for linter infrastructure |
