...
| `task_delegation_nudge.py` | `TaskDelegationNudgeHook` | `BeforeTurnHook`, `BeforeIterationHook` | before_turn, before_iteration | Behavior nudge: reminds a subagent-owning agent to dispatch. Self-gating (`task` tool registered + non-empty live roster + no recent `task` usage). `before_turn` sets `TASK_NUDGE_PENDING`; `before_iteration` pops it on first evaluation of the attempt and injects the reminder into history before the LLM request is built. Never touches the continuation flags. Roster-referenced via HOOK-slot factories (`task_delegation_nudge`) |
| `todo_planning_nudge.py` | `TodoPlanningNudgeHook` | `BeforeTurnHook`, `BeforeIterationHook` | before_turn, before_iteration | Behavior nudge: reminds a todo-owning agent to plan. Self-gating (`todo_write` registered + todo store completely empty for the session + no recent todo tool usage). Same `TODO_NUDGE_PENDING` pop-on-evaluate state machine; `todo_store=None` (poolless harness) skips silently. Roster-referenced via HOOK-slot factories (`todo_planning_nudge`) |
