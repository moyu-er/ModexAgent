...
| `task_delegation_nudge.py` | `TaskDelegationNudgeHook` | `BeforeTurnHook`, `BeforeIterationHook` | before_turn, before_iteration | **DEPRECATED** (poor effectiveness; no longer roster-referenced by shipped declarations — factories stay registered for custom opt-in). Behavior nudge: reminds a subagent-owning agent to dispatch. Self-gating (`task` tool registered + non-empty live roster). `before_turn` arms `TASK_NUDGE_PENDING` per attempt; `before_iteration` pops it on first evaluation — gate failures and a `USED` verdict (`scan_tool_usage_in_turn`: `task` called within the current turn's 3-assistant window) settle without injection; `SHORT_TURN` (fewer than 3 assistant messages since the last user/agent message — includes fresh-turn entry) re-arms for later iterations; `DUE` (3 assistant steps without usage) injects once and settles. Never touches the continuation flags. If revived: arm from `start_node_turn` instead of `before_turn` |
| `todo_planning_nudge.py` | `TodoPlanningNudgeHook` | `BeforeTurnHook`, `BeforeIterationHook` | before_turn, before_iteration | **DEPRECATED** (poor effectiveness; no longer roster-referenced by shipped declarations — factories stay registered for custom opt-in). Behavior nudge: reminds a todo-owning agent to plan. Self-gating (`todo_write` registered + todo store completely empty for the session + no `todo_write`/`todo_read` usage in the current turn's 3-assistant window). Same pop-on-evaluate verdict machine via `TODO_NUDGE_PENDING` (`SHORT_TURN` re-arms, `USED`/gates settle, `DUE` injects once); `todo_store=None` (poolless harness) skips silently. If revived: arm from `start_node_turn` instead of `before_turn` |
| `length_guard.py` | `LengthGuardHook` | `AfterLLMResponseHook`, `AfterTurnHook` | after_llm_response, after_turn | Recovers degenerate turn endings and fails honestly on exhaustion. `after_llm_response` records `custom[LAST_LLM_FINISH_REASON]` and resets `custom[LENGTH_GUARD_NUDGES]` to 0 on any productive response (content or tool calls). `after_turn` acts only on `COMPLETED` results whose last response was degenerate: `LENGTH`/`STOP` with empty content and zero tool calls, or `LENGTH` with truncated prose — injects a no-thinking nudge system-reminder (`NUDGE_NO_OUTPUT` / `NUDGE_TRUNCATED`) and sets `CONTINUATION_REQUEST` + `CONTINUATION_RENEW_MAX_TURNS` so the gate re-enters the ReAct loop past `MAX_TURNS`. After `MAX_NUDGES=10` (Final, not configurable) consecutive degenerate endings with no progress, mutates the turn's `AgentResult` in place to `StopReason.ERROR` instead of completing silently (AgentResult is non-frozen; AfterTurnNode dispatches AFTER_TURN with the same object it wrote to `state.result`). Tree-agnostic; registered in `register_tree_aware_hooks` (default priority) |
