<!-- Parent: ../AGENTS.md -->
<!-- Updated: 2026-09-10 | tool-group compile manifests -->
...
| ``PositionDefaults`` / ``defaults_for_position`` / ``effective_defaults`` / ``memory_config_for_position`` | ``defaults.py`` | The SPEC §3.2 position-derived defaults table: root → archive/core memory family (Experience is NOT in any preset — the ``experience`` capability is its sole enablement) + approval eligibility + eager registration + toolset ``full``; non-root → session-only + lazy + ``read_write``. The dead legacy ``tool_preset`` values land here |
...
| ``compile_scope`` / ``ScopeCompilation`` / ``CompiledAgent`` / ``AgentProvenance`` | ``compiler.py`` | The pure compiler: validated tree → per-agent ``AssemblySpec`` + ``EffectiveAgentConfig`` + ``AgentProvenance``. Runs C0/C1/C2 capability compilation and carries capability-contributed atomic ``ToolGroupSpec`` manifests when their anchor survives roster merge. It validates unique non-overlapping groups, exact anchor membership, and rejects independent companion edits. Manifests are contextual candidate variants, not runtime selection reports. Provenance bills expose each group under its anchor and suppress duplicate scalar rendering of that anchor. Same inputs → byte-identical outputs. |
...
- Atomic group companions never enter declaration edits independently. Configure or veto the anchor; assembly selects one manifest-approved exact variant.
