...
| ``AgentSpec`` | ``spec.py`` | Unified per-node agent declaration (the legacy Main/Sub type split is deleted, ticket 11). Root-ness derives from ``parent is None`` — never declared. Field face: ``toolset`` (``None`` = position-derived), ``tool_supplements``, ``tools``, ``hooks``, ``memory`` (``MemoryDeclaration``), ``approval`` (root-only, V9), ``eager``, ``execution_strategy``/``provider_kind`` |
...
| ``AgentOverlay`` / ``PoolOverlay`` / ``ScopeOverlay`` / ``apply_scope_overlay`` | ``overlay.py`` | Frozen, closed-schema pre-compile overlays. Purely transforms a loaded ``ScopeSpec`` (peer stripping, agent filtering, tool/memory/prompt/approval overrides) before the unchanged boot validation and compilation path |
| ``compile_scope`` / ``ScopeCompilation`` / ``CompiledAgent`` / ``AgentProvenance`` / ``ToolReplacement`` | ``compiler.py`` | The pure compiler: validated tree → per-agent ``AssemblySpec`` + ``EffectiveAgentConfig`` (the V6 input face) + ``AgentProvenance`` (per-field winning layer, per-tool origin, O3 same-name replacement records ``edit ← aci``). Same inputs → byte-identical outputs. Injects the derived communication entries (``task``/``send_to_agent``/``send_to_peer``) into ``tools``; keeps the legacy ``pool_name`` = root-agent-name convention |
...
- The derived communication entries are compiler-injected (SPEC §5.2): ``task`` for agents with declared children (direct children only), ``send_to_agent`` for every non-root, ``send_to_peer`` for roots of pools with links. They are never roster-declared and never materialize-time side-registered — resolution happens via the TOOL-slot FW factories in ``plugins/defaults/communication.py``.
...
- ``modex_agent.tools.presets`` — ``ToolPreset``/``ToolSupplement``
