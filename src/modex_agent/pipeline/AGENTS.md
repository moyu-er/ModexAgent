...
| File | Description |
|------|-------------|
| `pipeline.py` | `AgentPipeline` slimmed facade (~347L, 13 params — ADR-0025 D4). Main loop: `run()` → `_process_message()` (route/dedup/busy-mode/lock) → delegates the locked turn to `TurnRunner.process_locked()`. Public turn entry: `process_message()` (historical `AgentResult | None`), mapped from the typed producer `process_message_outcome()` → `TurnOutcome` — the single fact source pool dispatch consumes. Request-scope seam: `terminate_pending_approval()` delegates to the turn runner (audited, LLM-free batch termination). Session queries: `is_session_active()`/`has_active_sessions()`/`get_active_turn_uuid()` (delegate to `TurnSessionRegistry`). Lifecycle: `run()`/`stop()`/`cleanup_session_resources()`. Owns the `_dream_task` + `DreamScanner`. 11 backward-compat read-only delegation properties expose turn_runner internals for code that reads them. The 5 mutable mirror setters are deleted (ADR-0025 D4). |
| `turn_runner_abc.py` | `TurnRunner` ABC (ADR-0025 D3) — the seam between `AgentPipeline` and concrete turn runners. 1 abstract method (`process_locked`) + 3 lifecycle methods with no-op defaults (`cleanup_session`, `load_pending_approval`, `bind_to_pipeline`) + 1 lifecycle no-op `terminate_pending_approval()` (request-scope cancellation contract) + 2 post-construction wiring methods (`set_pool_context`, `set_emitter_factory`) + 12 read-only properties with `None` defaults. `ReActTurnRunner` overrides all; `ExternalTurnRunner` overrides only `set_emitter_factory`. See ADR-0025 D3 deviations for why the surface is larger than the original "one method" spec. |
...
| `turn_outcome.py` | `TurnOutcome` (frozen, kind/payload-validated: FINISHED carries `result`, SUSPENDED carries `suspension`, HANDLED carries neither) + `TurnSuspension` (`turn_uuid` + `ApprovalRequestView` batch) + `TurnOutcomeKind` — the single typed producer of turn facts; `AgentPipeline.process_message` maps it to the historical contract, pool dispatch notes it into the request-scope owner. |
| `approval_resumer.py` | `ApprovalResumer` — pure approval state machine. `load_pending()` queries `TurnStateStore` for `SUSPENDED` turns; `apply_resume()` applies a decision (matched by owner-minted `approval_id`; a stale identity is dropped upstream without save or re-render, or by `tool_call_id` for webui precision), re-saves partial state, and restores `agent_context.runtime.state` on completion. `terminate_pending()` terminates a pending batch WITHOUT any tool run — audits DENIED by `DecisionActor.REQUEST_CANCEL` and deletes the snapshot. **Single-direction dependency**: no knowledge of turn execution — the caller (`TurnRunner`) drives execute + delete_turn + drain. |
