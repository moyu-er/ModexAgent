...
Owns command-pattern, path-boundary, optional traversal and SSRF checks,
plus the ``WorkspacePolicy`` file-boundary projection. The approval
...
# guard_path.py / guard_traversal.py). Everything else is a deny rule.
_TRAVERSAL_CATEGORY = "path_traversal"
...
class SecurityDecisionService:
    ...
    provider. The static guard layers (command-pattern deny rules,
    traversal, SSRF) compile once — they are root-independent. The two
    root-dependent projections (the ``PathBoundaryGuard`` command
    envelope and the ``WorkspacePolicy`` file boundary) are rebuilt from
    ``root_provider.current()`` on EVERY evaluation, so a workspace
    switch updates the envelope immediately. Known file-tool checks use
    the workspace plus declared writable roots, including for reads;
    DANGER_FULL_ACCESS declares "no boundary", so no file-boundary check
    is built at all (not ``enforce=False`` — the check simply does not
    exist).
    """
    ...
    def _build_static_pipeline(self) -> GuardPipeline:
        """Compile the root-independent guard chain (deny + traversal).

        The built-in deny rules are structural: ``CommandPatternGuard``
        is always installed — no supported config disables it (the
        ``guard.enabled`` toggle gates only the advisory layers below).
        """
        guards: list[CommandGuard] = [CommandPatternGuard()]
        if self._settings.guard.enabled and self._settings.guard.path_traversal:
            guards.append(PathTraversalGuard())
    ...
    def _live_boundary_pipeline(self) -> GuardPipeline | None:
        ...
        if self._settings.policy is SandboxPolicy.DANGER_FULL_ACCESS:
            return None
        allow_roots = [str(r) for r in self._settings.writable_roots]
    ...
    def _live_file_policy(self) -> WorkspacePolicy | None:
        """File-tool boundary projection; None under DANGER_FULL_ACCESS."""
        if self._settings.policy is SandboxPolicy.DANGER_FULL_ACCESS:
            return None
        root = self._live_root()
        writable = (
            (root, *self._settings.writable_roots)
            if self._settings.policy is SandboxPolicy.WORKSPACE_WRITE
            else ()
        )
        return WorkspacePolicy(
            WorkspacePolicyConfig(
                root=str(root),
                allow_paths=tuple(str(canonicalize_path(r, base=root)) for r in self._settings.writable_roots),
                writable_paths=tuple(str(r) for r in writable),
            )
        )
    ...
    def evaluate_command(self, command: str) -> GuardVerdict:
        """Best-effort command/input check: hard deny, traversal, SSRF, boundary.

        Hard findings precede the approvable boundary category. This does
        not interpret arbitrary scripts, expansions, or accumulated stdin;
        CLEAN is not a claim of kernel containment.
        """
        if not command:
        ...
        if boundary is not None:
            ...
            if not boundary_result.allowed:
                verdict = self._verdict_from_pipeline(boundary_result)
                if verdict.category is GuardCategory.BOUNDARY:
                    root = self._live_root()
                    roots = PathEnvelope((root, *self._settings.writable_roots), base=root).roots
                    return verdict.model_copy(update={"allowed_roots": roots})
                return verdict
        return GuardVerdict(category=GuardCategory.CLEAN)
    ...
    def _verdict_from_pipeline(self, result: GuardResult) -> GuardVerdict:
        ...
        for match in result.matches:
            if match.category == _TRAVERSAL_CATEGORY:
                return GuardVerdict(category=GuardCategory.TRAVERSAL, reason=result.reason)
            if match.category == _BOUNDARY_CATEGORY:
                return GuardVerdict(category=GuardCategory.BOUNDARY, reason=result.reason)
    ...
    def evaluate_file_tool(self, name: str, path: str | None) -> GuardVerdict:
        """Judge a file-tool invocation against the live policy projection.

        Order: READ_ONLY refuses write tools outright (DENY_RULE — a
        hard policy refuse, never approvable; see module docstring),
        then the live workspace boundary (BOUNDARY — the gray zone).
        Missing required targets deny; DANGER_FULL_ACCESS has no boundary.
        Protected subpaths are read-only even inside writable roots.

        ``name`` selects the read/write descriptor and labels denials.
        """
        ...
        try:
            ...
            target = path
        if (
            descriptor.effect is ToolEffect.WRITE
            and self._settings.policy is SandboxPolicy.READ_ONLY
        ):
            return GuardVerdict(
                category=GuardCategory.DENY_RULE,
                reason=f"write tool {name} refused under read-only policy",
                target=target,
                allowed_roots=roots,
            )
        if path is None:
            return GuardVerdict(category=GuardCategory.DENY_RULE, reason="missing file target", allowed_roots=roots)
        policy = self._live_file_policy()
        if policy is None:
            return GuardVerdict(category=GuardCategory.CLEAN)
        if descriptor.effect is ToolEffect.WRITE:
            protected = PathEnvelope(
                tuple(r / sub for r in roots for sub in self._settings.protected_subpaths),
                base=root,
            )
            if protected.contains(path, base=root):
                return GuardVerdict(
                    category=GuardCategory.DENY_RULE, reason="protected path is read-only",
                    target=target, allowed_roots=roots,
                )
        try:
            policy.require_within(path)
        ...
        match descriptor.effect:
            ...
            case ToolEffect.EXECUTE | ToolEffect.EXECUTION_INPUT:
                ...
                if working_dir is not None:
                    return self.evaluate_file_tool("read", working_dir if isinstance(working_dir, str) else None)
                return verdict
            case ToolEffect.WEB:
                if target.url:
