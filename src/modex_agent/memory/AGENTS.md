...
| `compaction_governance.py` | `MemoryCompactionGovernance` — read-side (pre-LLM) trigger face of `compact_session`: resolves the turn's budget via `budget.resolve_effective_budget`, drives persistent compaction, and rebuilds `[system?] + ctx.to_messages()` from the refreshed history. |
| `budget.py` | `ContextBudget` + `resolve_effective_budget` — the sole definition of the per-turn budget priority chain (`ModelInfo.context_limit/max_output_tokens` → static pool config). |
...
| `cleanup.py` | `cleanup_session()`, `CleanupResult` — 5-phase pipeline: trigger+boundary → compact generation → session commit (`[compact_summary]`+`[tail]`) → pruned catalog write (topic from compact's `## Objective`) → archive generation (optional, default off; archive state advances atomically inside this phase, DreamEngine polling is the only archive-consolidation trigger). Takes the turn's `ContextBudget` (limit + output reservation in one typed value); the tail keep budget is absolute: `clamp(usable × 0.25, 2000, 15000)` (PRD §4.4.7; there is no ratio knob). The budget is handed to the `SessionCompactorAgent` so its summary calls size against the CURRENT model's window (single pass, or sliding-window map + reduce inside the compactor). `CleanupResult` carries `tokens_before`/`tokens_after` (char-estimated via `TokenEstimator`) for savings/thrash metrics. |
