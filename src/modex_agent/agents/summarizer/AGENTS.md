...
Summarizer and file-scoped ReAct agents for offline memory processing. `SessionCompactorAgent` performs a tool-less single LLM call; `ArchiveSummarizer` and `CoreMemoryConsolidator` specialize `ScopedFileAgent`.
