"""Framework ABC for building external subagent instances.

``SubagentExternalBuilder`` is the seam ``AgentTemplate.materialize``
dispatches to when the subagent spec's ``execution_strategy`` is
``EXTERNAL``. It is intentionally an ABC: the framework does not know
how to assemble a concrete external subagent (provider backend,
session store, env builder, …); that assembly is the business layer's
responsibility (T8 wires a concrete implementation into
``AgentMaterializeDeps.subagent_external_builder``).
...
class SubagentExternalBuilder(ABC):
    """Framework ABC that builds an :class:`AgentInstance` for an external subagent.

    Independent of the main agent's factory path —
    :attr:`AgentMaterializeDeps.subagent_external_builder` is an optional
    field, injected only by pools that declare at least one external subagent;
    react-only pools leave it ``None``.

    Implementations own the full assembly of the external subagent
    ...
    across the react and external branches.
    """
    ...
    async def build(
        ...
        """Build an :class:`AgentInstance` for an external subagent.
        ...
        Args:
            ...
            descriptor: The :class:`AgentDescriptor` already constructed by
                ...
                ``execution_strategy=EXTERNAL``, ``provider_kind`` from
...
__all__ = ["SubagentExternalBuilder"]
