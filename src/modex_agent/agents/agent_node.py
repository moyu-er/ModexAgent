...
from modex_graph.integration import (
...
class _AgentInputIntegrator(DefaultInputIntegrator):
    def integrate(self, payloads: list[IntegratedPayload]) -> IntegratedInput:
        fresh_payloads = [
            payload
            for payload in payloads
            if payload.status == DeliverConsumptionStatus.PENDING
        ]
        return super().integrate(fresh_payloads)
...
class AgentNode(Node[Any], ABC):
    ...
    Agent session memory (ReAct MessageStore) persists upstream input as
    SYSTEM_REMINDER across invocations. Its input integrator therefore filters
    ``CONSUMED_PENDING`` payloads so crash recovery does not duplicate input
    already present in the agent's session history.

    See ADR-0038 decision 5 for the full rationale.
    """

    DESCRIPTION_NOT_FOUND = "[not found]"
    input_integrator = _AgentInputIntegrator()
