...
class ToolParameter:
    ...
    properties: Optional[Dict[str, "ToolParameter"]] = None  # 用于 OBJECT 类型
    additional_properties: Optional[bool] = None  # 是否允许额外属性
...
# 尝试导入 Literal
try:
    from typing import Literal, get_args as get_literal_args
    HAS_LITERAL = True
except ImportError:
    try:
        from typing_extensions import Literal, get_args as get_literal_args
        HAS_LITERAL = True
    except ImportError:
        HAS_LITERAL = False
        Literal = None
...
def _extract_description_from_annotation(annotation: Any) -> Optional[str]:
    """
    从注解中提取描述信息。
...
def _get_literal_values(type_hint: Any) -> Optional[List[str]]:
    """获取 Literal 类型的所有可能值"""
