...
def _make_fake_ctx(*, interceptor_chain=None):
    ...
    state = ReActTurnState(
        identity=TurnIdentity(agent_id="test", session=SessionId.from_str("test-session-001"), turn_id="t1"),
    ...
    ctx = AgentContext(
        ...
        tool_manager=InMemoryToolManager(), session=SessionId.from_str("test.agent"),
        max_iterations=3,
