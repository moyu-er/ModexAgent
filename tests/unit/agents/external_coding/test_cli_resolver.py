"""Unit tests for :mod:`modex_agent.agents.external_coding.cli_resolver`.
...
from modex_agent.agents.external_coding.cli_resolver import (
...
class TestHasModexctl:
    ...
    def test_returns_true_when_runnable_present(
...
class TestSiblingBinDir:
    def test_returns_scripts_on_windows_layout(self, tmp_path: Path) -> None:
        ...
        with mock.patch.object(sys, "executable", str(fake_python)):
            with mock.patch("modex_agent.agents.external_coding.cli_resolver.sys.platform", "win32"):
    ...
    def test_returns_executable_parent_on_posix(self, tmp_path: Path) -> None:
        ...
        with mock.patch.object(sys, "executable", str(fake_python)):
            with mock.patch("modex_agent.agents.external_coding.cli_resolver.sys.platform", "linux"):
                result = _sibling_bin_dir()
...
class TestResolveModexctlBinDir:
    ...
    def test_strategy_1_stale_override_falls_through_to_strategy_2(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_strategy_1_override_without_modexctl_falls_through(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_strategy_2_sibling_of_sys_executable(self, tmp_path: Path) -> None:
        ...
        env_without_override = {
            k: v for k, v in os.environ.items() if k != "MODEXBOT_BIN_DIR"
        }
    ...
    def test_strategy_4_raises_with_diagnostic_context_when_all_fail(
        self, tmp_path: Path
    ) -> None:
