...
class RecordingEmitter(ContentEmitter[ExternalCodingEvent]):  # type: ignore[type-arg]
    ...
    async def emit(self, event: ExternalCodingEvent, data: object | None = None) -> None:
...
class TestStreamingProviderBackendABC:
    def test_is_subclass_of_provider_backend(self) -> None:
        from modex_agent.agents.external_coding.contracts import ProviderBackend
...
class TestScriptedStreamingAdapter:
    ...
    async def test_emits_one_emission_per_parsed_line(self, tmp_path: Path) -> None:
        ...
        assert received[0].event is ExternalCodingEvent.TEXT_DELTA
...
class TestOutboundSendViaRouting:
    ...
    async def test_side_effect_calls_t2_build_inbox_line(self, tmp_path: Path) -> None:
        ...
        assert line["content"] == "hello from agent1"
...
class TestExternalCodingAgentBuilder:
