...
Covers:
  - ``with_child_discovery_sink`` (+ the other 3 setters) thread the
    collaborators through ``build()`` onto the ``ExternalCodingAgent``.
...
class TestBuilderChildDiscoveryFluentApi:
    ...
    def test_build_without_child_collaborators_defaults_to_none(
        self, tmp_path: Path
    ) -> None:
        backend_provider, store, spec = _required_collaborators(tmp_path)
    ...
    def test_build_agent_without_child_collaborators_defaults_to_none(
        self, tmp_path: Path
    ) -> None:
        backend_provider, store, spec = _required_collaborators(tmp_path)
        descriptor = AgentDescriptor(
