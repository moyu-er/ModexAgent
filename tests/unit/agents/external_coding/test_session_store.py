"""Unit tests for ExternalSessionStore (T4)."""
...
from modex_agent.agents.external_coding import ExternalPaths, ProviderKind, SessionMapEntry
...
class TestExternalSessionStoreFresh:
    def test_resolve_unknown_sid_returns_none(self, tmp_path: Path) -> None:
        store = ExternalSessionStore(_paths(tmp_path))
...
class TestExternalSessionStoreCommit:
    @pytest.mark.asyncio
    async def test_commit_then_resolve_returns_resume(self, tmp_path: Path) -> None:
        store = ExternalSessionStore(_paths(tmp_path))
        await store.acommit("modex-1", "provider-1", ProviderKind.PI)
    ...
    async def test_commit_overwrites_existing_entry(self, tmp_path: Path) -> None:
        store = ExternalSessionStore(_paths(tmp_path))
        await store.acommit("modex-1", "provider-1", ProviderKind.PI)
        await store.acommit("modex-1", "provider-2", ProviderKind.PI)
        provider_sid, is_resume = store.resolve("modex-1")
    ...
    async def test_commit_preserves_other_entries(self, tmp_path: Path) -> None:
        store = ExternalSessionStore(_paths(tmp_path))
        await store.acommit("modex-1", "provider-1", ProviderKind.PI)
        await store.acommit("modex-2", "provider-2", ProviderKind.OPENCODE)
    ...
    async def test_commit_supports_all_provider_kinds(self, tmp_path: Path) -> None:
        store = ExternalSessionStore(_paths(tmp_path))
        await store.acommit("modex-pi", "p-pi", ProviderKind.PI)
        await store.acommit("modex-oc", "p-oc", ProviderKind.OPENCODE)
        assert store.resolve("modex-pi")[0] == "p-pi"
        assert store.resolve("modex-oc")[0] == "p-oc"


class TestExternalSessionStoreInvalidate:
    @pytest.mark.asyncio
    async def test_invalidate_then_resolve_returns_none(self, tmp_path: Path) -> None:
        store = ExternalSessionStore(_paths(tmp_path))
        await store.acommit("modex-1", "provider-1", ProviderKind.PI)
        await store.ainvalidate("modex-1")
        provider_sid, is_resume = store.resolve("modex-1")
        assert provider_sid is None
        assert is_resume is False
    ...
    async def test_invalidate_unknown_sid_is_noop(self, tmp_path: Path) -> None:
        store = ExternalSessionStore(_paths(tmp_path))
        await store.ainvalidate("never-existed")
    ...
    async def test_commit_after_invalidate_recovers(self, tmp_path: Path) -> None:
        ...
        await store.acommit("modex-1", "provider-1", ProviderKind.PI)
        await store.ainvalidate("modex-1")
        await store.acommit("modex-1", "provider-new", ProviderKind.PI)
        provider_sid, is_resume = store.resolve("modex-1")
        assert provider_sid == "provider-new"
        assert is_resume is True


class TestExternalSessionStoreRoundTripViaSessionMapEntry:
    @pytest.mark.asyncio
...
class TestExternalSessionStoreConcurrentCommits:
    @pytest.mark.asyncio
    async def test_concurrent_commits_same_sid_no_torn_write(self, tmp_path: Path) -> None:
        paths = _paths(tmp_path)
        store = ExternalSessionStore(paths)
        await asyncio.gather(
            store.acommit("modex-1", "provider-A", ProviderKind.PI),
            store.acommit("modex-1", "provider-B", ProviderKind.PI),
        )
        raw = json.loads(paths.session_map().read_text(encoding="utf-8"))
        assert "modex-1" in raw
    ...
    async def test_concurrent_commits_different_sids_both_persist(self, tmp_path: Path) -> None:
        ...
        await asyncio.gather(
            store.acommit("modex-1", "provider-A", ProviderKind.PI),
            store.acommit("modex-2", "provider-B", ProviderKind.OPENCODE),
            store.acommit("modex-3", "provider-C", ProviderKind.PI),
        )
        raw = json.loads(paths.session_map().read_text(encoding="utf-8"))
    ...
    async def test_concurrent_commit_and_invalidate_resolves_consistently(
        ...
        await asyncio.gather(
            store.acommit("modex-1", "provider-A", ProviderKind.PI),
            store.ainvalidate("modex-1"),
        )
        raw = json.loads(paths.session_map().read_text(encoding="utf-8"))
        assert "modex-1" in raw


class TestExternalSessionStoreExternalLock:
    @pytest.mark.asyncio
    async def test_shared_lock_serialises_two_stores(self, tmp_path: Path) -> None:
        paths = _paths(tmp_path)
        lock = asyncio.Lock()
        store_a = ExternalSessionStore(paths, lock=lock)
        store_b = ExternalSessionStore(paths, lock=lock)
        await asyncio.gather(
            store_a.acommit("modex-1", "provider-A", ProviderKind.PI),
            store_b.acommit("modex-2", "provider-B", ProviderKind.OPENCODE),
        )
        assert store_a.resolve("modex-1")[0] == "provider-A"
        assert store_b.resolve("modex-2")[0] == "provider-B"


class TestExternalSessionStorePersistenceAcrossInstances:
    @pytest.mark.asyncio
    async def test_second_store_sees_committed_entries(self, tmp_path: Path) -> None:
        paths = _paths(tmp_path)
        writer = ExternalSessionStore(paths)
        await writer.acommit("modex-1", "provider-1", ProviderKind.PI)
        reader = ExternalSessionStore(paths)
