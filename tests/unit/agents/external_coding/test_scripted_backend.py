...
class TestScriptedProviderBackendExecution:
    ...
    @pytest.mark.asyncio
    async def test_completed_status_when_programme_empty(
        self, tmp_path: Path
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_records_opts_first_before_side_effects(
        self, tmp_path: Path
    ) -> None:
...
class TestScriptedProviderBackendProgramme:
    ...
    @pytest.mark.asyncio
    async def test_playback_step_count_matches_programme(
        self, tmp_path: Path
    ) -> None:
...
class TestScriptedProviderBackendSideEffect:
    ...
    @pytest.mark.asyncio
    async def test_no_side_effect_call_means_nothing_invoked(
        self, tmp_path: Path
    ) -> None:
        invocations = 0
    ...
    @pytest.mark.asyncio
    async def test_side_effect_invoked_exactly_once_when_marked(
        self, tmp_path: Path
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_side_effect_invoked_at_each_marked_step(
        self, tmp_path: Path
    ) -> None:
        invocations = 0
    ...
    @pytest.mark.asyncio
    async def test_side_effect_can_safely_call_send_routing(
        self, tmp_path: Path
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_side_effect_no_callable_registered_is_silent(
        self, tmp_path: Path
    ) -> None:
...
class TestScriptedProviderBackendCallList:
    ...
    @pytest.mark.asyncio
    async def test_calls_entries_are_pydantic_instances(
        self, tmp_path: Path
    ) -> None:
        backend = ScriptedProviderBackend(ScriptedProgramme())
        await backend.execute(ExecOptions(prompt="a", workdir=tmp_path))
