...
async def test_publish_delivers_to_all_subscribers(broker):
    ...
    msg = BrokerMessage(payload={"content": "hi all"}, sender=Address("user", "u1"), topic="chat:general")
...
async def test_broadcast_reaches_all_registered_consumers(broker):
    ...
    msg = BrokerMessage(payload={"content": "broadcast"}, sender=Address("system", "sys"), broadcast=True)
...
async def test_consume_stream_iterates_messages(broker):
    addr = Address("agent", "streamer")
    await broker.register_consumer(addr)

    for i in range(5):
        await broker.send_to(addr, BrokerMessage(payload={"i": i}, sender=Address("user", "u1"), recipient=addr))
...
async def test_subscribe_multiple_topics(broker):
    ...
    await broker.publish("topic:a", BrokerMessage(payload={"content": "a1"}, sender=Address("user", "u1"), topic="topic:a"))
    await broker.publish("topic:b", BrokerMessage(payload={"content": "b1"}, sender=Address("user", "u1"), topic="topic:b"))
    await broker.publish("topic:a", BrokerMessage(payload={"content": "a2"}, sender=Address("user", "u1"), topic="topic:a"))
...
async def test_unregister_consumer_stops_delivery(broker):
    addr = Address("agent", "temp")
    await broker.register_consumer(addr)
    await broker.unregister_consumer(addr)

    msg = BrokerMessage(payload={"content": "x"}, sender=Address("user", "u1"), recipient=addr)
    await broker.send_to(addr, msg)
