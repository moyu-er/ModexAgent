...
class TestDeliverStoreCRUD:
    ...
    def test_query_consumable_returns_pending(self, kind: str) -> None:
        ...
        result = store.query_consumable(_GRAPH_INSTANCE_ID, "node_a")
        assert len(result) == 2
    ...
    def test_different_graph_instances_isolated(self, kind: str) -> None:
        ...
        assert len(store.query_consumable(_GRAPH_INSTANCE_ID, "a")) == 1
        assert len(store.query_consumable(_OTHER_GRAPH_INSTANCE_ID, "a")) == 1
    ...
    def test_content_round_trip_dict(self, kind: str) -> None:
        ...
        result = store.query_consumable(_GRAPH_INSTANCE_ID, "a")
...
class TestSqliteDeliverStoreSpecifics:
    def test_create_table_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = store2.query_consumable(_GRAPH_INSTANCE_ID, "a")
            assert len(result) == 1
