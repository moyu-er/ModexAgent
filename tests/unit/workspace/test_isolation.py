...
``framework.workspace`` must never import ``bot.*`` or name a concrete business
resource (pool, agent pool, memory system, ...). Business code plugs in via
:class:`framework.workspace.factory.ResourceFactory`.
"""
...
PKG = Path(__file__).resolve().parents[3] / "framework" / "workspace"
...
# foundational). ``framework.pipeline.snapshot`` is allowed: ``resources.py``
...
def test_framework_package_has_no_business_imports() -> None:
    ...
    assert not offenders, f"business coupling in framework.workspace: {offenders}"
