"""Unit tests for pool_builder external dispatch and availability gate.

These tests exercise the bot-layer wiring without booting the whole service or
spawning a real external CLI.  They verify:

* ``execution_strategy == "external"`` causes the pool builder to
  construct an :class:`ExternalAgent` when the provider executable is on
  PATH.
...
@pytest.mark.asyncio
async def test_external_pool_builds_agent_when_provider_available(
    tmp_path: Path,
) -> None:
    """When the provider executable is found, the main agent is an ExternalAgent."""
    pool_instance = await _build_external_pool(tmp_path, which_result="/usr/bin/opencode")
    ...
    assert agent._provider_kind.value == "opencode"


@pytest.mark.asyncio
async def test_external_pool_skips_agent_when_provider_missing(
    ...
    assert any(
        "external provider 'opencode' not found on PATH" in r.message
...
@pytest.mark.asyncio
async def test_external_pool_boots_without_model_yml(
    tmp_path: Path,
) -> None:
    ...
    agent = pool_instance.pool._agents["ext"].pipeline.agent
    assert isinstance(agent, ExternalAgent)
    # No provider is built for external pools.
