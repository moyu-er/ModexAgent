...
def _platform_shell_info() -> ShellInfo:
    if sys.platform == "win32":
        return ShellInfo(family=ShellFamily.BASH, path=r"C:\Windows\System32\bash.exe", platform=Platform.WINDOWS)
    import shutil
    for name, family in [("bash", ShellFamily.BASH), ("zsh", ShellFamily.ZSH), ("sh", ShellFamily.SH)]:
...
def _make_manager(visibility=TerminalVisibility.HIDDEN):
    ...
    cfg = TerminalRuntimeConfig(default_command_timeout_seconds=5)
...
async def test_command_reuses_default_tab_when_session_id_set() -> None:
    ...
    try:
        ...
        assert 'conv123.main' not in names, (
            f"CommandTool created a new tab named after sessionId. "
            f"Expected ['mytab'], got {names}"
        )
