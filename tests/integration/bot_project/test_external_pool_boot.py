"""Integration tests for bot-layer external pool boot.

Validates T10 acceptance:
- pool_builder dispatches ``execution_strategy: external`` to the
  external builder.
...
from tests.integration.bot_project._external_fixtures import (
    _MockInputAdapter,
    _MockOutputAdapter,
    _MockProvider,
)
...
@pytest.mark.asyncio
async def test_pool_pi_boots_with_external_strategy(
    ...
    with (
        patch.object(BotService, "_project_dir", property(lambda self: bot_service_config.parent)),
        patch(
        ...
        patch("shutil.which", side_effect=_fake_which(provider_available=True)),
    ):
        service = BotService(
...
async def test_pool_pi_skips_main_agent_when_provider_missing(
    ...
    with (
        ...
        patch("bot.service.builders._load_agent_mcp_tools", return_value=([], None)),
        patch("shutil.which", side_effect=_fake_which(provider_available=False)),
    ):
        service = BotService(
            config_dir=bot_service_config,
            input_adapter=mock_input_adapter,
        ...
        try:
            ...
            # external pools don't register send_to_agent (the external
...
async def test_default_pool_can_send_to_pool_pi_inbox(
    ...
    with (
        ...
        try:
            ...
            assert any(s.endswith(".pi") for s in pending), (
