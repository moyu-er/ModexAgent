...
- External coding agents (Pi, OpenCode) participate in ADR-0019 peer topology as NORMAL main agents of their own dedicated pools (`pool_pi`, `pool_opencode`). They communicate back through the `modexctl send` CLI (writes directly to target pool's `pending.jsonl` with XML-wrapped `<agent_message>` content), not through `send_to_agent` (which they do not have). Other agents talk to them via the standard `send_to_agent` tool. See ADR-0022 and `docs/design/external-coding-agent-integration/`.
