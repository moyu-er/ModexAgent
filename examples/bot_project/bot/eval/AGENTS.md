...
| `cli.py` | Typer CLI — `curate`, `run` (--toolset/--mode), `record-golden`, `replay-golden`, `metrics`, `compare` (404 graceful degradation) |
| `__init__.py` | Package marker |

## Key Constraints

- **Separate process**: the bot runtime never imports `bot.eval`; eval constructs its own agent instances via `agent_harness`
- **No bot_config.yml changes**: all eval config via code/CLI; `eval_score_injection` stays default `false`
