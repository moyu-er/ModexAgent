...
- **Envelopes and transcripts take pool from the emitter's constructor-owned value.** `WebBotEmitter` stores `pool` at construction (inherited `BotTranscriptEmitter.__init__`); transcript appends and `DeltaEnvelope.pool` use only that value, and `None` becomes `""`, i.e. no self-reporting (`emitter/bot_transcript.py`, `emitter/web_bot.py`). Ownership is fixed by the factory at emit time; there is no per-emit pool lookup.
