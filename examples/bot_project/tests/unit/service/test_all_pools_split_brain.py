"""Declared-pool boot road — coder + review pools.

The ticket-11 split-brain goldens were removed: shipped ``bot.yml`` is
user-customizable configuration, and unit tests must not pin its contents.
What remains verifies the boot MECHANISM on the declared road: the derived
communication registration (``comm_tools_derived``) and the tree-derived
``task`` / ``send_to_peer`` presence.
"""
