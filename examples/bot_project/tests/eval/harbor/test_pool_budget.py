...
class _CostedProvider(LLMProvider):
...
class _StreamingCostedProvider(StreamingLLMProvider):
...
def test_budgeted_provider_does_not_mask_streaming_capability() -> None:
    """Regression weld: the budget wrapper must BE a StreamingLLMProvider.

    The ReAct LLM client gates its streaming path (chunk-level
    dispatch-deadline renewal — the fix for long single LLM calls dying as
    "hung" at the dispatch timeout) on isinstance(provider,
    StreamingLLMProvider). A plain-LLMProvider wrapper silently forced
    every budgeted provider onto the non-streaming path, making the eval
    emitter's wants_streaming() opt-in inert.
    """
...
async def test_budgeted_provider_chat_stream_synthesizes_delta_for_plain_delegate() -> None:
    provider = BudgetedProvider(_CostedProvider(0.2), _pricebook(), _budget_config())
    content_deltas: list[str] = []
