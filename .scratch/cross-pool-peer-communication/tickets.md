...
Reference: spec at `.scratch/cross-pool-peer-communication/PRD.md`; ADR-0019 (`docs/adr/0019-cross-pool-peer-communication.md`) + blueprint (`docs/adr/0019-cross-pool-peer-communication-blueprint.md`).
...
- [ ] `_load_per_agent_mcp` relocated to `modex_agent/tools/` (as `load_per_agent_mcp`, dropping the leading underscore — it is now a public resident of its rightful module).
- [ ] Import in `multi_agent/template.py` updated to the new path.
- [ ] Three test cases in `tests/framework/tools/test_mcp_registry_wiring.py` updated to import from the new path.
- [ ] Stale path reference in `tools/mcp_adapter.py` comment updated.
- [ ] `pytest tests/framework/tools/test_mcp_registry_wiring.py -v` — all three cases pass unchanged.
- [ ] `ruff check src/modex_agent/multi_agent/communication.py` — no import of the moved function remains.
- [ ] `mypy src/modex_agent/tools/` — type checks clean.
...
- [ ] `CommunicationTarget` has `pool_name: str = ""` and `bus_ref: AgentMessageBus | None = None` fields with defaults that preserve every existing constructor call.
- [ ] `CommunicationTargetStore.add` raises `ValueError` on duplicate name, listing both existing and new `pool_name` values in the message.
- [ ] `CommunicationTargetStore.get(name) -> CommunicationTarget | None` method added (trivial dict lookup).
- [ ] `bus_ref` type annotation uses `TYPE_CHECKING` guard to avoid circular import at runtime.
- [ ] Existing unit tests `tests/unit/multi_agent/test_communication_target_store.py` pass unchanged (they don't add duplicate names).
- [ ] New unit test: `test_communication_target_store_rejects_duplicate_name` — two targets with same name, assert `ValueError` with both pool names in message.
- [ ] New unit test: `test_communication_target_get_returns_target_or_none` — existing name returns the target, unknown name returns `None`.
- [ ] Audit `_build_communication` in `examples/bot_project/bot/service/pool_builder.py` — confirm no subagent name collides with a profile name today (if one does, it was a latent bug the silent-drop masked).
- [ ] `mypy src/modex_agent/multi_agent/tools.py` — type checks clean.
...
- [ ] `multi_agent/communication.py` replaced by `multi_agent/communication/` package with: `__init__.py` (re-exports), `service.py` (thin orchestrator), `topology.py` (`TopologyPolicy.check`), `result.py` (`AgentSendResult` + ack formatting), `strategies/base.py` (`SendStrategy` ABC + `SendRequest` / `SendDeps` bundles), `strategies/subagent_dispatch.py`, `strategies/parent_reply.py`.
- [ ] `TopologyPolicy.check(sender_kind, target, context) -> str | None` is the single enforcement point — no topology checks remain in the send trunk.
- [ ] `AgentCommunicationService._send` reduced to: store-lookup → topology gate → strategy dispatch → `strategy.execute(request)`.
- [ ] Strategy selection is a flat dispatch on `bus_ref` presence + `target.kind`, not a nested if-tree.
- [ ] `subagent_dispatch` strategy preserves today's behaviour: fresh-uuid session prefix, `TASK_REQUEST` type, invocation_id surfaced in ack, `record_send` tracker, trace/output paths.
- [ ] `parent_reply` strategy preserves today's behaviour: parent session reuse, `AGENT_MESSAGE` type, `acknowledge` + `acknowledge_received` bracket close.
- [ ] `SendToAgentTool.execute` calls `store.get(name)` and passes `CommunicationTarget` to `service.send_async(target=target, ...)`.
- [ ] `AgentCommunicationService.__init__` signature backward-compatible — existing wiring in `pool_builder.py::_build_communication` works unchanged.
- [ ] `from modex_agent.multi_agent.communication import AgentCommunicationService, AgentSendResult` resolves unchanged (re-export).
- [ ] `pytest tests/integration/multi_agent/test_pool_communication.py -v` — passes unchanged (behaviour preservation).
- [ ] `pytest tests/unit/multi_agent/test_communication_service.py -v` — passes unchanged.
- [ ] `pytest tests/unit/multi_agent/test_send_to_agent_tools.py -v` — passes (minimal adaptation for target-object passing is acceptable).
- [ ] New unit tests: `tests/unit/multi_agent/communication/strategies/test_subagent_dispatch.py` and `test_parent_reply.py` — each strategy tested in isolation with mocked `SendDeps`, verifying observable outputs (envelope shape, session shape, result fields).
- [ ] `mypy src/modex_agent/multi_agent/communication/` — type checks clean.
...
- [ ] `PeerNormalStrategy` implements `SendStrategy`: `build_session` uses `create_with_prefix(prefix=sender_session_prefix, agent_name=target.name)` with no `parent_session_id`; `normalize_invocation_id` returns sender prefix internally but `AgentSendResult.invocation_id` is `None`; `build_envelope` uses `AGENT_MESSAGE` type with `invocation_id` on envelope but `build_agent_message` called with `invocation_id=None`; `apply_tracker` calls `record_send` (not `acknowledge`); `deliver` reads `target.bus_ref or local_bus` and calls `bus.send(...)`.
- [ ] Service strategy dispatch selects `peer_normal` when `target.bus_ref is not None`.
- [ ] `InboxPoller` registers an unseen session id in its local `SessionRegistry` before dispatching (generic branch — handles any unseen session, not just peer-originated).
- [ ] Integration test `tests/integration/multi_agent/test_cross_pool_peer.py` (pattern: `test_multi_pool_isolation.py` two-pool setup):
...
- [ ] `pytest tests/integration/multi_agent/ -v -m integration` — all existing pool communication + isolation tests still pass.
...
- [ ] `PoolTree` (or the appropriate business config structure) gains `peers: list[str] = []` field.
- [ ] `pool.yml` top-level `peers:` key parsed into `PoolTree.peers`.
- [ ] Framework `PoolConfig` (`ioc/configs/pool.py`) unchanged — verify no diff.
- [ ] `PoolStore.write_pool` (or a dedicated validator) enforces: peer name exists as a pool; bidirectional invariant (A lists B ⟺ B lists A). Violations raise `PoolValidationError`.
- [ ] Extended tests in `examples/bot_project/tests/bot/config/test_pool_store.py` (or a new `test_peer_pool_config.py`):
  - `pool.yml` with `peers: [coding, research]` parses into `PoolTree.peers`.
  - Half-edge (A lists B, B does not list A) → `PoolValidationError`.
  - Dangling peer name (references non-existent pool) → `PoolValidationError`.
  - Round-trip: write A with `peers: [B]`, write B with `peers: [A]`, read both back, verify both peers lists correct.
...
- [ ] `PoolInstance` dataclass exposes `agent_bus: AgentMessageBus` and `target_store: CommunicationTargetStore` as readable fields, populated during `create_pool`.
- [ ] Phase 2 post-assembly runs after all Phase 1 `create_pool` calls complete — iterates each pool's `peers` list, constructs peer `CommunicationTarget` entries with `bus_ref`, calls `local_store.add(target)`.
- [ ] Insertion-order invariant: subagent targets (Phase 1) ahead of peer targets (Phase 2) in `store.list()`.
- [ ] Integration test `examples/bot_project/tests/integration/test_peer_pool_assembly.py`:
  - Workspace with two pools, each listing the other in `peers`.
  - Run Phase 1 + Phase 2 assembly.
  - Pool A's store contains target named B's main agent with `bus_ref` pointing at B's bus.
  - Pool B's store contains reciprocal target.
  - Subagent targets ahead of peer targets in `store.list()` order.
  - Duplicate-name collision (two pools with same main agent name) → `ValueError` during Phase 2.
- [ ] `pytest examples/bot_project/tests -q` — all existing bot tests pass.
...
- [ ] WebUI pool config UI supports adding a peer relationship between pool A and pool B in a single action that writes both `pool.yml` files.
- [ ] Removing a peer relationship removes both sides atomically.
- [ ] Peer config UI displays the peer pool's main agent name (resolved from peer's `main_agent_name`), not just the pool directory name.
- [ ] Frontend optimistic update: adding A→B immediately shows B→A in B's peer list.
- [ ] Backend write is transactional — both files written before commit, or neither.
- [ ] Manual or automated verification: add peer edge via UI → both `pool.yml` files updated → restart → both pools' `send_to_agent` tools list each other's main agent.
