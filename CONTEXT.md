...
**NodeResult**:
The structured return value of `Node.execute`. A frozen Pydantic value object with two optional fields: `transition: str | None` (action string for static edge lookup — a node returns this to say "I finished, route via edges whose `reason` matches"), `state_update: Mapping[str, Any] | None` (declarative state update applied to channels, also serves as the payload delivered to downstream nodes). A node may alternatively mutate state imperatively (`ctx.state.x = y`) — the engine syncs Pydantic fields to channels at snapshot time. Both styles coexist (Z-style dual-mode state access, ADR-0033). Under parallel scheduling, imperative mutations on a forked state do NOT propagate to the parent — only `state_update` merges back via channel semantics.
_Avoid_: node output (too generic — NodeResult is the typed return), transition (that is one field of NodeResult)

**Command**:
The dynamic routing primitive — a frozen Pydantic value object with `goto` (str | list[str] | list[Task] for dynamic target / multi-target parallel fan-out). When a node returns `NodeResult(command=Command(goto=...))`, the command overrides static-edge routing entirely. `list[str]` triggers all targets in parallel (shared state); `list[Task]` triggers all tasks in parallel (each with independent state via `Task.state`). State updates live on `NodeResult.state_update`, not on `Command` (separation of routing from state mutation). The external `route_fn` conditional-edge mechanism is removed — all conditional routing is done via `transition` + static edges or `Command.goto`.
_Avoid_: directive (too generic), routing command (collides with slash commands)

**Channel**:
The per-field state semantics on a `GraphState`. Each field annotated with `Annotated[T, ChannelSpec]` is backed by a `BaseChannel` instance that owns its own update / checkpoint / restore behavior. Two channels ship: `LastValue` (single-writer, default — raises `InvalidUpdateError` on ≥2 writes in one parallel superstep per ADR-0034) and `ReducerChannel` (binary operator fan-in — folds all concurrent writes). The `BaseChannel` ABC is the extension seam. Channels are the snapshot substrate: `GraphState.checkpoint()` returns per-channel JSON; `from_checkpoint()` restores each channel independently.
_Avoid_: slot (too generic), field (that is the Pydantic field; the channel is its state-semantics backing)

**GraphRuntime**:
The framework-defined ABC that lets a `Node` invoke AOP concerns (hooks, snapshot, control drain, governance, approval suspension) without the Graph Engine knowing their concrete types. Lives in the Graph Engine package as an ABC with no-op default methods. `modex_agent` provides `ReactGraphRuntime` (and similar) implementations that bridge to `HookRunner` / `SnapshotPolicy` / `ControlChannel` / `ApprovalRuntime`. A standalone graph user supplies `GraphRuntime()` (no-op defaults) or their own subclass. The engine auto-invokes `before_node` / `after_node` (node-level universal lifecycle); iteration-level hooks (`BEFORE_ITERATION` / `AFTER_ITERATION`) are dispatched explicitly by ReAct nodes via `ctx.runtime.dispatch_hook(...)`, NOT engine-auto-invoked, because "iteration" is a ReAct concept, not a universal graph concept. The `around` method routes `ITERATION` scope only — `TOOL_CALL` and `LLM_STREAM` are node-local AOP invoked directly via `InterceptorChain` (`around_tool_call` / `around_llm_stream`) because their typed contexts are not constructible from `GraphContext` (ADR-0033 D5).
_Avoid_: runtime services (that is `AgentRuntimeServices`), runtime context (too generic), graph services

**GraphBubbleUp**:
The control-flow-via-exception family for the Graph Engine. Base class `GraphBubbleUp(Exception)`; concrete subclasses `GraphInterrupt` (HITL suspend, raised by `ctx.interrupt(value)`), `InvalidUpdateError` (channel multi-write violation under parallel scheduling, raised by `LastValue` when ≥2 writes occur in one superstep per ADR-0034), `GraphDrained` (cooperative shutdown, raised at superstep boundary when a drain is requested — class exists, wiring deferred), `ParentCommand` (subgraph→parent routing — class exists, wiring deferred). The engine never swallows these; they propagate to the caller (typically `TurnRunner.process_locked`).
...
A predecessor node that was actually selected by an upstream routing decision (static edge, conditional edge, or `Command.goto`) and will therefore execute in the current graph run. Contrast with a *declared* predecessor — a node connected by an edge that may or may not be traversed depending on conditional routing. Trigger Mode readiness is computed against activated predecessors only, never against all declared predecessors, so that a conditional branch skipping one arm does not leave a downstream join waiting forever for a predecessor that will never fire.
...
The `ParallelScheduler` execution model (ADR-0034 D2): instances start as independent `asyncio.Task`s the moment their dependencies are satisfied — no batch barrier (`asyncio.gather`). A short task (10ms) completing does not wait for a long task (10s) in the same batch. Contrast with BSP supersteps (langgraph's model), where a barrier waits for all tasks in a step to complete before merging. Continuous scheduling uses fork-based state isolation (ADR-0034 D7) + generation-based conflict detection (ADR-0034 D8/D18) to replace the consistency guarantee that BSP's step-immutability invariant provides.
_Avoid_: event-driven scheduling (too generic), task-parallel (overloaded with distributed computing sense)

**Ready**:
The state of a node instance whose Trigger Mode is satisfied: for `ON_ALL_PREDS`, all activated predecessors have dispatched AND reachability is clear (no active instance can reach this node via outgoing edges); for `ON_RECEIVE`, a dispatch has been received (no reachability check). A READY instance is immediately forked and launched as an `asyncio.Task` under continuous scheduling.
_Avoid_: runnable (too generic), available (collides with channel availability)

**Scheduler**:
The pluggable scheduling strategy behind `GraphEngine`. An ABC with two implementations: `LinearScheduler` (Phase a behavior — single-node pointer, sequential execution, no fork, no instance tracking; the default) and `ParallelScheduler` (continuous scheduling, multi-instance, fork-based state isolation, dispatch abstraction, trigger modes, reachability-based readiness for ON_ALL_PREDS; opt-in via `Graph.compile(scheduler="parallel")`). Both share the same graph topology (`CompiledGraph`), `Node`/`NodeResult`/`Command` interfaces, and `GraphRuntime` AOP bridge. See ADR-0034.
...
**Node Instance**:
One execution of a node under `ParallelScheduler`. Identified by `{node_name}#{global_seq}` (e.g. `body#0`, `body#1`). Each instance has an independent lifecycle (DORMANT → PENDING → READY → RUNNING → COMPLETED), a forked state snapshot (forked at PENDING→READY transition), and a `fork_version` (the main_state generation it forked from). Loops produce new instances rather than resetting state. `ON_RECEIVE` multi-trigger produces multiple concurrent instances. The instance is immutable after COMPLETED — re-activation (by a loop edge) creates a fresh instance with a new sequence number.
_Avoid_: node execution (too generic — that is the act, not the identity), node state (collides with `GraphState`)

**Dispatch**:
The framework-level primitive for sending work from one node to another. `ctx.dispatch(target, state_update)` records a dispatch event, validates the target is reachable via the current node's outgoing edges (edge whitelist), and updates the target's readiness. `NodeResult.transition` and `Command.goto` are declarative syntax sugar compiled into dispatch calls after `execute` returns. Dispatch takes effect immediately (not deferred to `execute` return) — supporting modexctl CLI-driven dispatch where an agent inside `execute` triggers downstream work before the node completes. The interface is designed to be remotely callable (future IPC adapter for modexctl). See ADR-0034 D5.
...
A group of node instances that forked the same main_state snapshot. When multiple instances become READY simultaneously (before any merge occurs), they share the same `fork_version` and belong to one generation. Two instances in the same generation writing the same `LastValue` field = conflict (`InvalidUpdateError`). An instance in a later generation writing a field that an earlier generation wrote = sequential overwrite (not a conflict — the later instance saw the earlier one's merged result). Tracked by `WriteConflictDetector` (ABC, ADR-0034 D18); default impl `GenerationWriteTracker`.
_Avoid_: wave, batch, superstep (those are scheduling concepts; generation is a conflict-detection concept)

**WriteConflictDetector**:
The ABC (ADR-0034 D18) that detects concurrent write conflicts for `LastValue` fields under continuous scheduling. Replaces the batch-barrier conflict detection (where `apply_concurrent_updates` sees all writes at once). Methods: `register(fork_version)` on instance fork, `commit(fork_version, fields)` before merge (raises `InvalidUpdateError` on same-generation field collision), `complete(fork_version)` after merge, `advance()` to increment main_state version. Default implementation: `GenerationWriteTracker`. All methods are synchronous — the caller must invoke commit+apply+advance+complete as one atomic segment with no `await`.
_Avoid_: conflict resolver, merge policy (the detector only detects; resolution is via channel semantics)

**CheckpointStore**:
The ABC (ADR-0034 D19) that persists scheduler state for crash recovery. Saves `CheckpointData` (main_state checkpoint dict + version + pending ON_ALL_PREDS queue + completed instances + dispatch events) after each instance merge. Async — does not block the scheduling loop. On crash, recovery starts from the last saved checkpoint. Default impl: `SqliteCheckpointStore`. The `ConcurrentWriteTracker` state is NOT persisted (it tracks only in-flight instances; on restart, no instances are running).
_Avoid_: snapshot store (collides with `SnapshotStore` in the AOP layer), state store (too generic)
...
**Core Memory** (formerly *Knowledge layer*, renamed by ADR-0035):
...
**KnowledgeBase** (introduced by ADR-0035, designed by forthcoming ADR-0036):
