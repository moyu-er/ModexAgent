...
**Capability** (ADR-0047):
The unit of the 11th `ComponentSlot` (`CAPABILITY`) and the **ownership unit for an optional feature spanning tools/hooks/sections/supply — including its implementation**: a plugin-provided bundle that packages components which belong together (tool names plus same-name tool replacements, hook names, prompt sections, a pool-level supply, per-agent wiring) **and the feature's own implementation** (models, sources, prompts, config) as one vertical slice — not assembly glue with the implementation scattered elsewhere. Registered behind one name that is also the declaration key under `capabilities:`; that map is the enablement authority. Five phases: `applies`/`contribute`/`bind` run inside `compile_scope` as deterministic pure functions (the spec-hash byte-stability contract covers them); `supply` runs at pool assembly; `assemble` runs per agent at assembly. Contributed names flow into the existing `tools`/`hooks` rosters and resolve through the existing TOOL/HOOK slots — there is no second component-resolution path. Five FW-bundled packages shipped (`aci`, `ast_grep`, `todo`, `experience`, `subagents`), with Experience and Skills converging into full vertical-slice capability packages (`plugins/defaults/capabilities/{experience,skills}/`, per the architecture-convergence migration); the framework knows only the protocol, never a capability name.
_Avoid_: plugin (that is the registration unit), feature flag (a capability is a compile-time bundle, not a runtime toggle), supplement (the retired `ToolSupplement` face), wiring bundle (a capability owns the feature implementation, not just its registration)
