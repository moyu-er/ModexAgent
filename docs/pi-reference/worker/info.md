...
| **Can communicate with parent?**            | Yes — via `contact_supervisor`. Escalates blocked decisions to the parent agent.         |
| **Can communicate with sibling subagents?** | No. Star topology: all communication routes through the main agent.                      |
| **Who receives its output?**                | The parent agent. Implementation summary + changed files are returned to the main agent. |
