...
5. **Agent node input idempotency**: `AgentNode` sets a custom `input_integrator` (`_AgentInputIntegrator`, a `DefaultInputIntegrator` subclass) that filters `IntegratedPayload.status == "pending"`, dropping `CONSUMED_PENDING` payloads. The engine's `Node._integrate_upstream` is inherited unchanged — it collects all consumable delivers (`PENDING` + `CONSUMED_PENDING`), marks them consumed, and hands the full payload list to `input_integrator.integrate()`. The `IntegratedPayload.status` field exists precisely so business integrators can filter by consumption status; the engine default (`DefaultInputIntegrator`) does not filter. Agent session memory (ReAct `MessageStore`) persists upstream input across invocations — crash recovery must not re-inject already-consumed delivers. `BotAgentNode.execute` detects re-execution (session has existing messages) and skips `[Origin Request]` duplication, injecting only new upstream input.
...
- `AgentNode.input_integrator` (`_AgentInputIntegrator`) diverges from the engine's `DefaultInputIntegrator` — it filters `IntegratedPayload.status == "pending"`, dropping `CONSUMED_PENDING` payloads that the default integrator would pass through. This is a justified integrator substitution, not a `_integrate_upstream` override: agent nodes have a second persistence layer (session memory) that the engine does not know about. Re-consuming already-injected delivers would duplicate `SYSTEM_REMINDER` messages in the agent's session history.
