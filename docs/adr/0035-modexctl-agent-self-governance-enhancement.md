...
> The CLI architecture decisions in this ADR — direct SQLite inbox delivery,
> env-gated quadrant routing, and local history queries — are superseded by
...
considered but deferred (see "Future Work — Cancellation").
...
### D1 — Disable rich rendering across all commands
...
ANSI color highlights remain (Typer's default) — they are harmless to
...
**structural noise** (box characters `─│┌┐└┘`, multi-line table layout,
...
### D2 — `send` gains `--invocation-id`, response surface, and quadrant-differentiated output
...
already received a `<replied>` for" — i.e., once the bot has processed the
...
match `MODEX_SESSION_ID` — these two cases are treated identically and not
...
a session it did not correspond to — that would silently mask an agent
...
(i.e., main → subagent dispatch within the same pool). For
`MODEX_COMM_KIND=normal` (cross-pool peer) and subagent → parent reply,
invocation_id is not a concept and the parameter is ignored.

#### D2.2 — Quadrant-differentiated response output
...
Subagent will run asynchronously — wait for the <replied> block, do not poll.
```
Identical to ② minus `output_path` and `trace_dir` — external coding
...
**④ `subagent` → parent reply (subagent calling back to its parent)**
```
...
recipient's concern — telling the sender about it leaks implementation
details across the agent boundary.

**Not-found status rendering** (applies to ② and ③ when
...
### D3 — New `history` subcommand for inspecting subagent message history
...
- `--limit` (optional, default 3, max 10): number of messages to return.
  Values >10 are silently clamped to 10. Values ≤0 are rejected with a
  usage error.

The command is **only registered when `MODEX_COMM_KIND=subagent`** —
history inspection is a subagent-governance operation, not a peer
operation. (Peer main agents' history is out-of-scope: it would require
cross-workspace reads and is not part of this ADR.)

#### D3.2 — Data source and query
...
- No `state` filter — **soft-deleted messages (`state='soft_deleted'`)
...
`messages.jsonl` — that would require replicating
...
rather than a reimplementation — the helper owns the
...
#### D3.3 — VO (View Object) field filtering
...
`content` is preserved verbatim — its type is `str | list[ContentPart] | None`
...
`json.dumps(msg, ensure_ascii=False)` is used — it escapes embedded `\n`
...
order. This is consistent with "show me the latest N" semantics — the
first line is the most recent message.

**No header, no footer, no separator lines** — the entire stdout is
...
`InMemoryControlChannel` — a process-internal `dict[session_id →
dict[type → deque]]` guarded by an `asyncio.Lock`.
...
2. **Effective cancellation semantics at the ReAct loop level**. A
   ...
   directly — not a new problem, but one that must be documented in the
   CLI's response.
3. **`SubagentAutoSendHook` integration**. The hook already fires on
   ...
   normal `<replied>` channel — do not treat it as unsolicited" to
...
- Each piece above is non-trivial; (1a) alone is an ABC change plus a
  new persistence adapter plus a new poller — comparable in scope to
...
- **Quadrant output is parseable but not machine-typed.** The four
  ...
  parsing cost. `history` — the high-volume command — is already
  JSON Lines.)
