...
Phase 0-4 design complete. All specifiable decisions resolved and validated.
...
- Phase 3 (T13): **designed, not implemented** — execute rewrite (thin shell).
- Phase 4 (T14): **designed, not implemented** — TurnContextConfigPipeline + configurators.
...
- **Timeout for `tree.wait_quiesce`**: already implemented with `timeout: float` parameter (manager.py:219). Execute passes `self._node_timeout`. Noted in T13 "Open items".
