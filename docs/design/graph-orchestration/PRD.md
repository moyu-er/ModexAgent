...
- [共享知识库接口](issues/06-shared-knowledge-base-interface.md) — 业务功能增强,非框架级设计。modexctl kb CRUD加--by-task参数按taskId(=graph_instance_id)划区。带参数=task私有,不带=公共。补充deliver/submit不够表现上下文的场景。优先级低,框架+图实现完成后接入bot后再做。
...
- **transition/command 迁移**:deliver/submit 完全替代 transition/command。现有 modex_graph 代码(_compile_routing/NodeResult.transition/command/ctx.dispatch)需要迁移。这是实现阶段的工作,设计已完成。
