...
Source spec: `docs/design/external-coding-subagent/PRD.md`
Parent ADR: `docs/adr/0027-external-coding-agent-as-subagent.md`
Domain glossary: `CONTEXT.md` → "AgentImplementation", "SubagentSpec", "SubagentExternalCodingBuilder", "BackendProvider", "SubagentNotificationArtifactKind"
...
**What to build:** A bot operator can declare a subagent with `execution_strategy: external_coding` and `provider_kind: opencode` in `pool.yml`, and the schema rejects contradictory combinations (react + provider_kind, or external_coding + missing provider_kind). Existing pool configurations with no `execution_strategy` / `provider_kind` on subagents still validate unchanged. The `AgentImplementation` enum docstring is updated to mark `SUBAGENT + EXTERNAL` as supported rather than "reserved (future)".
...
- [x] `SubagentSpec` has `@model_validator(mode="after")` enforcing `provider_kind` set iff `execution_strategy == EXTERNAL_CODING`
...
- [x] `tests/unit/multi_agent/test_pool_config.py` covers: round-trip of new fields; validator rejects react + provider_kind; validator rejects external_coding + missing provider_kind; validator accepts external_coding + provider_kind
- [x] `AgentImplementation` docstring marks `SUBAGENT + EXTERNAL` as supported (no longer "reserved (future)")

## T2 — BackendProvider ABC + PoolScopedBackendProvider + ExternalCodingAgent migration  ✅ DONE

**What to build:** `ExternalCodingAgent` no longer holds a fixed `backend` — it borrows one per turn from a `BackendProvider`. The main-agent external-coding path (ADR-0022) wraps its pre-built backend in `PoolScopedBackendProvider` before passing it to the agent. Externally, main-agent behavior is byte-for-byte indistinguishable from pre-ADR-0027: same pool-scoped backend, same warm SSE reuse across turns, same `close()` at pool shutdown.
...
- [ ] `ExternalCodingAgent.__init__` signature: `backend` parameter replaced by `backend_provider: BackendProvider`
- [ ] `ExternalCodingAgent._run_turn` calls `provider.acquire()` at turn start and `provider.release(backend, turn_failed=...)` in a `finally` block
- [ ] `ExternalCodingAgent.stop()` calls `self._backend_provider.close_all()` instead of `self._backend.close()`
- [ ] `ExternalCodingAgentBuilder.build_agent` staticmethod updated: `backend` kwarg replaced by `backend_provider`
- [ ] `ExternalCodingAgentBuilder.build` fluent method updated: `with_backend` replaced by `with_backend_provider`
- [ ] Main-agent call site (`ExternalCodingAwareFactory.create_agent`) wraps its pre-built backend in `PoolScopedBackendProvider` before calling `build_agent`
- [ ] All existing `ExternalCodingAgent` / `ExternalCodingAgentBuilder` tests migrated to the new signature
- [ ] `tests/unit/agents/external_coding/test_agent.py` (Seam 3) verifies: `_run_turn` calls `acquire` at start, `release(turn_failed=...)` in finally, `turn_failed=True` on exception path
- [ ] Main-agent external-coding integration test (`tests/integration/multi_agent/test_cross_pool_external_coding.py`) still passes — behavior unchanged
...
- [ ] `tests/unit/agents/external_coding/test_turn_runner.py` (Seam 4 partial) verifies: hook fires exactly once on success; once on `Exception`; once on `CancelledError`; zero times when `hook_runner=None`
...
- [ ] `tests/unit/agents/external_coding/test_process_cleanup.py` (Seam 5) verifies: `weakref.finalize` fires on GC and kills subprocess (mock); `atexit` handler walks registry and kills; signal handler runs atexit; `close()` detaches finalizer (no double-kill); registered once (idempotent)

## T5 — SubagentExternalCodingBuilder ABC + AgentDescriptor.provider_kind + materialize dispatch  ✅ DONE

**What to build:** When `AgentTemplate.materialize` is asked to materialize a subagent whose `execution_strategy == EXTERNAL_CODING`, it forwards `execution_strategy` + `provider_kind` to the `AgentDescriptor` and dispatches to a new `SubagentExternalCodingBuilder` ABC instead of the existing `agent_factory.create_agent` path. The dispatch ends with the same `pool.register_resident` + `on_subagent_created` calls the react path makes, so parent-child wiring is uniform. React subagent materialize is byte-for-byte unchanged.

**Blocked by:** T1 (Schema — needs `SubagentSpec.execution_strategy` + `provider_kind` to forward)

- [ ] `SubagentExternalCodingBuilder` ABC defined in framework layer (`agents/external_coding/`) with single method `async def build(self, spec, descriptor, parent_session, invocation_id, deps) -> AgentInstance`
- [ ] `AgentMaterializeDeps` gains `subagent_external_coding_builder: SubagentExternalCodingBuilder | None = None` field
...
- [ ] `AgentTemplate.materialize` dispatches on `spec.execution_strategy == EXTERNAL_CODING` to `deps.subagent_external_coding_builder.build(...)`; else takes existing `deps.agent_factory.create_agent(...)` path
- [ ] External dispatch branch raises clear `ValueError` if `deps.subagent_external_coding_builder is None`
...
**What to build:** A `CachingBackendProvider` that caches warm backends (`OpenCodeServerBackend`) per-modex_session_id with an LRU cap, and shares a single stateless backend instance per `provider_kind` for per-turn-spawn backends (`OpenCodeBackend`, `PiBackend`). When the warm cache exceeds `MAX_WARM_BACKENDS` (default 10), the least-recently-used entry is evicted and its `opencode serve` process is killed via `close()`. This bounds the `opencode serve` process count regardless of how many modex sessions are active, and preserves warm SSE reuse for the common case of repeated turns on the same modex session.
...
- [ ] `tests/unit/agents/external_coding/test_backend_provider.py` (Seam 2) verifies: `PoolScopedBackendProvider` identity semantics; warm LRU eviction at cap; evicted backend's `close()` called; stateless sharing returns same instance; `acquire`/`release` pairing; `turn_failed=True` invalidation; `close_all` cleanup
...
- [ ] Business-layer `BotSubagentExternalCodingBuilder(SubagentExternalCodingBuilder)` implementation in `examples/bot_project/bot/service/`
...
- [ ] `build()` calls `ExternalCodingAgentBuilder.build_agent(descriptor, provider=None, backend_provider=..., session_store=..., parser=..., provider_kind=..., spec=...)`
...
- [ ] `pool_builder.create_pool` scans `pool_spec.subagents` for any `execution_strategy == EXTERNAL_CODING`; if found, constructs `BotSubagentExternalCodingBuilder` and injects it into `AgentMaterializeDeps.subagent_external_coding_builder`
- [ ] React-only pools (no external subagents) leave `subagent_external_coding_builder = None` — zero overhead
...
- [ ] `tests/unit/agents/external_coding/test_builder_external_coding.py` extended: `BotSubagentExternalCodingBuilder.build()` with mock collaborators verifies the full assembly
- [ ] `tests/integration/multi_agent/_external_coding_fixtures.py` extended: shared fixture for external-subagent scenario (scripted backend mock)
...
- [ ] Subagent materializes via `BotSubagentExternalCodingBuilder` (T8)
