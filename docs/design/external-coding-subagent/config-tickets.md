...
- [ ] A subagent spec with `execution_strategy: external_coding` + `provider_kind: opencode` round-trips: write → read → fields preserved
...
- Switching native→external sets `execution_strategy: "external_coding"` + `provider_kind: DEFAULT_EXTERNAL_PROVIDER` (preserving native field values in form state — they're hidden but not cleared, same as main agent)
...
- [ ] When `execution_strategy === "external_coding"`: Provider `DropdownPanel` + external-runtime panel render; native-only fields hidden
...
- [ ] `config/pools/coder/templates/worker.yml` has `execution_strategy: external_coding` + `provider_kind: opencode`
