...
## T4: memory_session_messages adapter + ColumnProjection 试点
...
- [ ] `SqliteMessageStore` defines `_MESSAGE_PROJECTION` with `message_id`, `role`, `content`+`is_content_json`, `token_count`
- [ ] `append_message` / `save_messages` use `_MESSAGE_PROJECTION.split(message)` for column extraction
- [ ] `load_messages` / `load_all_messages` use `_MESSAGE_PROJECTION.assemble(columns, json_str)` for dict reassembly
- [ ] `pin_message` / `unpin_message` / `delete_message` query `WHERE message_id = ?` (no `json_extract`)
- [ ] `memory_revisions` INSERT/UPDATE omit the `scope` column (only `scope_key`)
- [ ] No `time.time()` calls in `message_store.py` — all use `now_ms()`
- [ ] `tests/conformance/test_message_store_conformance.py` passes (round-trip: append → load returns semantically equal dict)
- [ ] `tests/conformance/test_message_row_state_conformance.py` passes (state machine: normal → pinned → soft_deleted)

---

## T5: inbox 表簇 adapter 整改
...
- [ ] `SqliteInboxMQ` defines `_INBOX_PROJECTION` with `message_id`, `message_type`, `session_id`
- [ ] `receive` / `deliver` INSERT into `inbox_messages` with only extracted columns + `payload_json`
- [ ] `_row_to_message` uses `_INBOX_PROJECTION.assemble()` (no manual column-by-column reconstruction)
- [ ] 5 `UPDATE inbox_topics SET state/last_active/message_count` calls deleted
- [ ] `inbox_topics` INSERT writes only `owner_scope_key`, `scope_key`, `session_id` (timestamps via DEFAULT)
- [ ] `inbox_delivered_ids` INSERT omits `session_id`
- [ ] `InboxMessage.timestamp` ↔ `created_at` int ms conversion at adapter boundary
- [ ] No `time.time()` calls — all use `now_ms()`
- [ ] `tests/conformance/test_inbox_mq_conformance.py` passes (receive → consume round-trip; exactly-once dedup; FIFO ordering)

---

## T6: 其他 memory 表 adapter 整改（kv/cursors/revisions/archive）
...
- [ ] `SqliteKVStore`: `scope` → `scope_key`, `time.time()` → `now_ms()`
- [ ] `SqliteCursorStore`: `scope` → `scope_key`, `time.time()` → `now_ms()`
- [ ] `SqliteArchiveStore` (entries + state): `scope` → `scope_key`, `time.time()` → `now_ms()`
- [ ] `memory_revisions` write path: `scope` column omitted (only `scope_key`)
- [ ] `tests/conformance/test_kv_store_conformance.py` passes
- [ ] `tests/conformance/test_cursor_store_conformance.py` passes
- [ ] `tests/conformance/test_archive_store_conformance.py` passes

---

## T7: turn_snapshots + sessions adapter 整改
...
- [ ] `SqliteTurnStateStore`: `scope` → `scope_key` in all SQL
- [ ] `SqliteTurnStateStore`: `now = time.time()` → `now = now_ms()`
- [ ] `SqliteTurnStateStore.save_turn`: `created_at` written as `int(snapshot.created_at * 1000)` (Phase 1 boundary conversion)
- [ ] `SqliteTurnStateStore._decode`: `snapshot_payload["created_at"] = row["created_at"] / 1000.0` before codec decode
- [ ] `SqliteTurnStateStore.list_active_turns`: `scope.created_before` converted to `int(scope.created_before * 1000)` for SQL
- [ ] `SqliteSessionStore`: `scope` → `scope_key` in INSERT/SELECT
- [ ] `SqliteSessionStore`: timestamp writes use `now_ms()` or DEFAULT
- [ ] No `time.time()` calls in either adapter
- [ ] `tests/conformance/test_turn_state_store_conformance.py` passes (save → load round-trip; Phase 1 boundary conversion transparent)
- [ ] `tests/conformance/test_session_store_conformance.py` passes

---

## T8: todos + approval_audit_log + external_session_map + pool_routing adapter 整改
...
- [ ] `SqliteTodoStore`: `scope` → `scope_key`, `time.time()` → `now_ms()`
- [ ] `SqliteApprovalAuditStore`: `scope` → `scope_key`, `decided_at` via `now_ms()`
- [ ] `SqliteExternalSessionMapStore`: `scope` → `scope_key`, `last_committed_at` via `now_ms()`
- [ ] `SqlitePoolRoutingStore`: `scope` → `scope_key`, `SELECT pool_name, pool` → `SELECT pool_name`
- [ ] No `time.time()` calls in any of the 4 adapters
- [ ] `tests/conformance/test_todo_store_conformance.py` passes
- [ ] `tests/conformance/test_approval_audit_store_conformance.py` passes
- [ ] `tests/conformance/test_external_session_map_store_conformance.py` passes
- [ ] `tests/conformance/test_pool_routing_store_conformance.py` passes
...
- [ ] `StorageRevision.updated_at` type is `int` (was `datetime`)
- [ ] `ArchiveEntry.created_at` type is `int` (was `datetime` or ISO string)
- [ ] `scoped_file.py`: `self._updated_at = now_ms()` (3 sites); archive entry `created_at` uses `now_ms()` (2 sites)
- [ ] `file.py`: `updated_at=now_ms()`; `now = now_ms()`
- [ ] `scoped_in_memory.py`: same as `scoped_file.py` (2 + 2 sites)
- [ ] `dir_archive.py`: `updated_at=now_ms()`; archive entry `created_at` uses `now_ms()`
- [ ] `memory/layers/archive.py`: `.isoformat()` calls on `created_at` removed (2 sites)
- [ ] `memory/default_system.py`: `.isoformat()` call on `created_at` removed
- [ ] `server_local.py`: `message.timestamp.isoformat()` → `int(message.timestamp.timestamp() * 1000)` (2 sites)
- [ ] `server_local.py`: `datetime.fromisoformat(data["timestamp"])` → `datetime.fromtimestamp(data["timestamp"] / 1000, tz=UTC)` (2 sites)
- [ ] `InboxMessage.timestamp` ABC contract stays `datetime`
- [ ] `ChatMessage.created_at` stays ISO-8601 string (unchanged)
- [ ] Conformance tests pass with file backend (assertions updated for int ms timestamps)

---

## T10: workspaces + session_workspace_map adapter + WorkspaceRecord 模型
...
- [ ] `WorkspaceRecord.created_at` and `.last_active` are `int` (ms epoch)
- [ ] `workspace/registry.py` and `workspace/store.py` use `now_ms()` (no `_now_iso()`)
- [ ] `SqliteWorkspaceRegistryStore` writes int-ms timestamps
- [ ] `SqliteWorkspaceRegistryStore` handles `metadata_json NOT NULL DEFAULT '{}'`
- [ ] `session_workspace_map` INSERT writes only `session_prefix` + `workspace_id` (timestamps via DEFAULT)
- [ ] No `time.time()` calls in `workspace_registry_store.py`
- [ ] `tests/conformance/test_workspace_registry_store_conformance.py` passes (assertions updated for int ms)

---

## T11: SqliteSessionDatabaseCleaner 整改
...
- [ ] `_SCOPE_DISCOVERY_SQL` uses `SELECT scope_key FROM {table}` (no `SELECT scope`)
- [ ] `_SCOPE_DELETES` uses `WHERE scope_key = ?` (no `WHERE scope = ?`)
- [ ] `if scope.pool is not None:` fallback deleted
- [ ] `_INBOX_CHILD_TABLES` does not contain `"inbox_dead_letter"`
- [ ] No `time.time()` calls — all use `now_ms()`
- [ ] `tests/unit/persistence/test_session_cleanup.py` passes (cascade delete across all 14 workspace tables; `inbox_dead_letter` cascade test removed; `scope` → `scope_key` in all assertions)
...
- [ ] `tests/conformance/test_sqlite_specific.py` passes against the new schema
- [ ] Smoke-test target is `sessions` (not `workspace_meta`)
- [ ] No `pool` generated-column checks
- [ ] Timestamp type checks assert `INTEGER`
- [ ] Trigger existence checks for every mutable table
- [ ] `inbox_dead_letter` and `workspace_meta` absence assertions
- [ ] `tests/architecture/` has new guard: no `workspace_meta`/`inbox_dead_letter` references in production code
- [ ] `tests/architecture/` has new guard: `RecordScope` has no `pool` field
- [ ] `tests/architecture/` has new guard: `utils/time.py` exports `now_ms`
...
- [ ] `pytest tests/conformance/ -v` — all 14 conformance test files pass (both backends)
- [ ] `pytest tests/unit/persistence/ -v` — all persistence unit tests pass
- [ ] `pytest tests/architecture/ -v` — all architecture guard tests pass
- [ ] `pytest examples/bot_project/tests/integration/ -v -m integration` — bot integration tests pass
- [ ] `bot_webui_transcript_events` table untouched; `sqlite_transcript_store.py` unchanged
- [ ] `PersistenceConfig.backend = FILE` then `SQLITE` produces semantically equivalent data for the same ABC operations
- [ ] No `time.time()` calls remain in `src/modex_agent/persistence/adapters/`
- [ ] No `scope` column references remain in `src/modex_agent/persistence/` (only `scope_key`)
- [ ] No `RecordScope(pool=...)` constructions remain in `examples/bot_project/bot/` (all use `BotRecordScope`)
- [ ] No `workspace_meta` or `inbox_dead_letter` references remain in production code
