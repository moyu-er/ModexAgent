...
The split is a design fact, not a defect to unify (ADR-0034 D2):
...
ADR-0033 D5.2 refinement note ("`around` routes `ITERATION` only; `TOOL_CALL` and `LLM_STREAM` are node-local") was already applied to the ADR's Status line during ADR-0034 writing — no further ADR edit needed.
...
- [ ] Comment added at `tool_executor.py` `InterceptorChain.around_tool_call` call site: "Canonical AOP path for `TOOL_CALL`. `ctx.runtime.around` is for `ITERATION` only — see ADR-0034 D2."
- [ ] Comment added at `llm_client.py` `InterceptorChain.around_llm_stream` call site: "Canonical AOP path for `LLM_STREAM`. `ctx.runtime.around` is for `ITERATION` only — see ADR-0034 D2."
